...
const data = [
  { key: "Technology", value: 38.1, color: "from-rose-200 to-pink-300" },
  { key: "Financials", value: 25.3, color: "from-sky-200 to-blue-300" },
  ...
  { key: "Utilities", value: 5.8, color: "from-emerald-100 to-emerald-200" },
      ...
      >
        ...
        {data.map((d, index) => {
          ...
          return (
            <ClientTooltip key={index}>
              ...
              <TooltipContent>
                <div>{d.key}</div>
                <div className="text-slate-500 text-sm">{d.value}</div>
              </TooltipContent>
            </ClientTooltip>
          );
        })}
          ...
          {xScale
            ...
            .map((active, i) => (
              <g
                ...
                className="text-slate-300/80 dark:text-gray-800/80"
                key={i}
              >
                <line
                  y1={0}
        ...
        {xScale.ticks(4).map((value, i) => (
          <div
            ...
            className="absolute text-xs -translate-x-1/2 tabular-nums text-slate-400"
          >
            {value}
          </div>
        ))}
      ...
      >
        {data.map((entry, i) => (
          <span
            ...
            className="absolute text-xs text-slate-400 -translate-y-1/2 w-full text-right"
          >
            {entry.key}
          </span>
        ))}
